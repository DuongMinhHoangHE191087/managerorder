...
export function NavUserMenu() {
  ...
  const displayName = user
    ? [user.firstName, user.lastName].filter(Boolean).join(" ") || user.email?.split("@")[0] || vi.userMenu.fallbackName
    : vi.userMenu.fallbackName;
      ...
      <div className="flex items-center gap-2 border-l border-[var(--border-soft)] pl-2 sm:gap-3 sm:pl-3">
        ...
        >
          <div className="text-right hidden sm:block">
            ...
            <p className="text-[10px] uppercase font-bold text-[var(--accent)] tracking-wider">{vi.userMenu.role}</p>
          </div>
