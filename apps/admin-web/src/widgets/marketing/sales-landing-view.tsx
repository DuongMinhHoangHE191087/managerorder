...
import {
  ...
  TRUST_STACK,
  getSalesVariantCopy,
  type OfferCard as MarketingOfferCard,
...
export function SalesLandingView({ variant, errorMessage, reset, resetLabel, offers }: SalesLandingProps) {
        ...
        <div className="overflow-hidden rounded-[32px] border border-white/80 bg-white/95 shadow-[0_24px_80px_-16px_rgba(0,0,0,0.15)] backdrop-blur-3xl">
          <div className="px-6 pb-5 pt-8 text-center sm:px-8">
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-black tracking-tight text-slate-800 leading-tight" translate="no">{BRAND.name}</h1>
                <p className="mt-1.5 bg-gradient-to-r from-blue-600 to-violet-600 bg-clip-text text-xs font-black uppercase tracking-[0.2em] text-transparent">
                  <span translate="no">{BRAND.tagline}</span> {vi.marketing.salesLanding.labels.official}
                </p>
              </div>
            </div>
          ...
          <section id="offers" className="bg-gradient-to-b from-white to-blue-50/30 px-6 py-6 sm:px-8">
            <div className="mb-4 flex items-center justify-center gap-2">
              ...
              <p className="text-xs font-black uppercase tracking-[0.15em] text-slate-800">
                {vi.marketing.salesLanding.labels.offerHeading}
              </p>
              <Sparkles className="size-4 text-amber-500" />
            </div>
            ...
            <p className="mt-4 flex items-center justify-center gap-1.5 text-center text-[11px] font-medium text-slate-500">
              ...
              {vi.marketing.salesLanding.labels.commitment}
            </p>
          </section>
          ...
          <section className="px-6 py-5 sm:px-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <ContactButton
                ...
                icon={MessageCircle}
                title={vi.marketing.salesLanding.contacts.zaloAdmin.title}
                subtitle={vi.marketing.salesLanding.contacts.zaloAdmin.subtitle}
              />
              <ContactButton
                href={CONTACTS.messenger}
                className="bg-gradient-to-br from-[#0084FF] to-[#0061fa] text-white shadow-lg shadow-blue-500/30"
                icon={Users}
                title={vi.marketing.salesLanding.contacts.messenger.title}
                subtitle={vi.marketing.salesLanding.contacts.messenger.subtitle}
              />
            </div>
            ...
            >
              <GroupDots />
              {vi.marketing.salesLanding.contacts.zaloGroup}
            </a>
          </section>
        </div>
          ...
          <div className="mt-5 flex animate-[fadeSlideUp_1s_ease-out_0.3s_forwards] justify-center opacity-0" style={{ transform: "translateY(0.5rem)" }}>
            <div className="flex items-center gap-2 rounded-full border border-white/60 bg-white/80 px-5 py-2.5 shadow-xl backdrop-blur-xl">
              ...
              <span translate="no">{BRAND.tagline}</span> {vi.marketing.salesLanding.labels.ecosystem}
              </span>
            </div>
          </div>
      </motion.div>
...
function OfferCard({ offer }: { offer: MarketingOfferCard }) {
  const href = offer.href || CONTACTS.zaloPersonal;

  return (
    <OfferLink href={href} className="group relative flex items-center gap-4 rounded-2xl border-2 border-slate-100 bg-white p-3 pr-4 shadow-sm transition-all duration-300 hover:border-blue-500 hover:shadow-xl hover:shadow-blue-500/10">
      {offer.tag === "HOT" ? (
        <div className="absolute -right-2.5 -top-2.5 animate-pulse rounded-full border border-white bg-rose-500 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-white shadow-lg">
          {vi.marketing.salesLanding.labels.hotBadge}
        </div>
      ) : null}
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-black leading-tight text-slate-800 transition-colors group-hover:text-blue-600">
          <span translate="no">{offer.label}</span>
        </p>
      ...
      <div className="flex flex-col items-end border-l border-slate-100 pl-2">
        <span className="mb-0.5 text-[10px] font-bold uppercase text-slate-400 line-through opacity-60">{vi.marketing.salesLanding.labels.originalPrice}</span>
...
function GroupDots() {
  return (
    <div className="flex -space-x-2">
      <div className="flex h-6 w-6 items-center justify-center rounded-full border border-white bg-blue-100 text-[8px] font-black text-blue-700">•</div>
      <div className="flex h-6 w-6 items-center justify-center rounded-full border border-white bg-green-100 text-[8px] font-black text-green-700">•</div>
      <div className="flex h-6 w-6 items-center justify-center rounded-full border border-white bg-purple-100 text-[8px] font-black text-purple-700">•</div>
    </div>
  );
}
