          ...
          <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col-reverse gap-2 sm:flex-row sm:gap-3">
              ...
              >
                ...
                {createPOMutation.isPending ? text.creating : text.create}
              </Button>
            </div>
          </div>
        }
        ...
        <div className="custom-scrollbar max-h-[65vh] space-y-5 overflow-y-auto pr-1">
          <div className="space-y-3">
            {poItems.map((item, idx) => (
              <div
                key={idx}
                className="space-y-3 rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-surface)] p-4"
              >
            ...
            <button
              type="button"
              onClick={addItem}
              className="cursor-pointer text-[12px] font-bold text-[var(--accent)] hover:underline"
            >
            ...
            <div className="mb-2 flex flex-wrap gap-2">
              {poProofImages.map((image, index) => (
                ...
                >
                  ...
                  <img
                    src={image}
                    alt={text.proofImageAlt(index + 1)}
                  ...
                  <button
                    ...
                    className="absolute right-0.5 top-0.5 cursor-pointer rounded-full bg-red-500 p-0.5 text-white opacity-0 transition-opacity group-hover:opacity-100"
                  >
                    <X className="size-3" />
                  </button>
                </div>
          ...
          <div className="space-y-3 border-t border-[var(--border-soft)]/60 pt-3">
            <button
              ...
              className={`flex cursor-pointer items-center gap-2 rounded-xl px-4 py-2 text-[12px] font-bold transition-all ${
                connectToInventory
            ...
            >
              ...
              <ChevronDown
                className={`ml-auto size-3.5 transition-transform ${connectToInventory ? "rotate-180" : ""}`}
              />
            </button>
          ...
          <div className="overflow-hidden rounded-xl border border-[var(--border-soft)] bg-[var(--bg-surface)]">
                ...
                .map((item, idx) => (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-2">
                      <Package className="size-3.5 shrink-0 text-[var(--accent)]" />
                      <span className="truncate text-[12px] font-bold text-[var(--fg-base)]">
                        {item.productName || text.productShortLabel}
                      </span>
                      <span className="text-[10px] text-[var(--fg-muted)]">x{item.quantity}</span>
                    </div>
                    <span className="ml-2 shrink-0 text-[12px] font-black text-[var(--fg-base)]">
                      {formatMoney(item.quantity * item.priceVnd)}
                    </span>
                  </div>
                ))}
