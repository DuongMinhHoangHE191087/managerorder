...
export function buildPurchaseOrderPaymentUpdatePayload(
  currentPO: Record<string, unknown>,
  amount: number,
  nowIso: string = new Date().toISOString(),
) {
  const currentTotalPaid = Number(
    currentPO.totalPaidVnd ?? currentPO.total_paid_vnd ?? 0,
  );
  const totalAmount = Number(
    currentPO.totalAmountVnd ?? currentPO.total_amount_vnd ?? 0,
  );
  const newTotalPaid = currentTotalPaid + amount;
  const fullyPaid = totalAmount > 0 && newTotalPaid >= totalAmount;
  const receivedAt = fullyPaid
    ? typeof currentPO.receivedAt === "string" && currentPO.receivedAt.trim()
      ? currentPO.receivedAt
      : typeof currentPO.received_at === "string" && currentPO.received_at.trim()
        ? currentPO.received_at
        : nowIso
    : undefined;
  ...
  return {
    updatePayload: {
      ...
      status: fullyPaid ? "received" : newTotalPaid > 0 ? "partial" : "pending",
...
export function usePurchaseOrderPayment(providerId: string) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { updatePayload, fullyPaid } = buildPurchaseOrderPaymentUpdatePayload(
        currentPO ?? {},
        amount,
      );
