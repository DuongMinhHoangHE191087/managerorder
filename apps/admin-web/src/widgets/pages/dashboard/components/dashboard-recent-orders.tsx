...
const DashboardRecentOrderRow = memo(function DashboardRecentOrderRow({ order }: DashboardRecentOrderRowProps) {
  const statusConfig = getOrderDisplayStatus(order.status);

  return (
    <tr data-testid="dashboard-recent-order-row" className="transition-colors hover:bg-[var(--surface-light)]/50">
      ...
      <td className="px-4 py-4">
        ...
        >
          <span className={`size-2 rounded-full ${statusConfig.dotPulse ? "animate-pulse" : ""}`} style={statusConfig.dotStyle} />
          {statusConfig.label}
        </span>
      </td>
      <td className="px-4 py-4 text-[13px] font-medium text-[var(--fg-muted)]">{formatDateLabel(order.createdAt)}</td>
  ...
  return (
    ...
    >
      ...
      {recentOrders.length === 0 ? (
        <div
          data-testid="dashboard-recent-orders-empty"
          className="p-8 text-center text-[13px] text-[var(--fg-muted)]"
        >
          ...
          <table className="w-full min-w-[780px] border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-[var(--border-soft)] bg-white">
              {recentOrders.map((order) => (
                <DashboardRecentOrderRow key={order.id} order={order} />
              ))}
            </tbody>
          </table>
        </div>
