...
export function ImportSummaryTable({ months = 6 }: { months?: number }) {
  ...
  const text = vi.dashboard.importSummary;
  ...
  return (
    <SlideUp delay={0.55} className="glass-card rounded-ios border border-[var(--border-soft)] shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      <div className="p-5 border-b border-[var(--border-soft)] bg-[var(--bg-app)]/50 backdrop-blur-sm">
        ...
        <p className="text-[12px] text-[var(--fg-muted)] mt-0.5">{months} {text.recentSuffix}</p>
      </div>
      ...
      <div className="grid grid-cols-3 text-center border-b border-[var(--border-soft)] bg-[var(--surface-light)]">
        <div className="p-3 border-r border-[var(--border-soft)]">
          <p className="text-[10px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">{text.totalOrders}</p>
          <p className="text-[14px] font-black text-[var(--fg-base)]">{summary.totalOrders}</p>
        </div>
        <div className="p-3 border-r border-[var(--border-soft)]">
          <p className="text-[10px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">{text.totalAmount}</p>
          <p className="text-[14px] font-black text-amber-600">{formatMoney(summary.totalAmountVnd)}</p>
        </div>
        <div className="p-3">
          <p className="text-[10px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">{text.avgPerOrder}</p>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-[var(--border-soft)] bg-[var(--bg-app)]/30">
              <th className="text-left px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.month}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.orderCount}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.importTotal}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.avgPerOrder}</th>
            </tr>
          </thead>
          <tbody>
            {data.data.map((row) => (
