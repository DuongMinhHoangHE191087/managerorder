...
export function ProductProfitTable({ days = 30 }: { days?: number }) {
  ...
  const text = vi.dashboard.productProfit;
    ...
    <SlideUp delay={0.5} className="glass-card rounded-ios border border-[var(--border-soft)] shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      <div className="p-5 border-b border-[var(--border-soft)] bg-[var(--bg-app)]/50 backdrop-blur-sm flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-[var(--fg-muted)] mt-0.5">{days} {text.recentSuffix}</p>
        </div>
        {summary.avgRoi !== null && (
      ...
      <div className="grid grid-cols-3 text-center border-b border-[var(--border-soft)] bg-[var(--surface-light)]">
        <div className="p-3 border-r border-[var(--border-soft)]">
          <p className="text-[10px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">{text.revenue}</p>
          <p className="text-[14px] font-black text-[var(--fg-base)]">{formatMoney(summary.totalRevenue)}</p>
        </div>
        <div className="p-3 border-r border-[var(--border-soft)]">
          <p className="text-[10px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">{text.cost}</p>
          <p className="text-[14px] font-black text-[var(--fg-base)]">{formatMoney(summary.totalCost)}</p>
        </div>
        <div className="p-3">
          <p className="text-[10px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">{text.profit}</p>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-[var(--border-soft)] bg-[var(--bg-app)]/30">
              <th className="text-left px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.product}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.quantity}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.revenue}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.cost}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.profit}</th>
              <th className="text-right px-4 py-3 font-bold text-[var(--fg-muted)] text-[11px] uppercase tracking-wider">{text.roi}</th>
            </tr>
          </thead>
          <tbody>
            {data.data.map((row) => (
              <tr key={row.productId} className="border-b border-[var(--border-soft)]/50 hover:bg-[var(--surface-light)] transition-colors">
                ...
                <td className="text-right px-4 py-3">
                  {row.roiPercent !== null ? (
                    ...
                    <span className="text-[11px] text-[var(--fg-muted)]">{text.emptyRoi}</span>
                  )}
                </td>
              </tr>
            ))}
