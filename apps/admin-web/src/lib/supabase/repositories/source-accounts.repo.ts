...
type SourceAccountOrderItemRow = Pick<OrderItemRow, "id" | "order_id" | "assigned_source_account_id" | "quantity">;
...
async function loadOrderItemForAccount(
  orderItemId: string,
  accountId: string,
): Promise<{ item: SourceAccountOrderItemRow; order: OrderBaseRow }> {
  const { data, error } = await supabase
    .from("order_items")
    .select("id, order_id, assigned_source_account_id, quantity")
    .eq("id", orderItemId)
    .single();
  ...
  const items = data as SourceAccountOrderItemRow;
  const ordersMap = await loadRowsByIds<OrderBaseRow>(
    supabase,
    "orders",
    accountId,
  ...
  const order = ordersMap.get(items.order_id);
  if (!order) {
    throw new Error("Order item does not belong to this account");
  }

  return { item: items, order };
}
...
export async function disconnectSourceAccount(sourceAccountId: string, orderItemId: string, accountId: string): Promise<void> {
  const { item } = await loadOrderItemForAccount(orderItemId, accountId);
  if (item.assigned_source_account_id !== sourceAccountId) {
    throw new Error("Order item is not connected to this source account");
  }
  ...
  const { error: itemError } = await supabase
    ...
    .eq('id', orderItemId)
    .eq('assigned_source_account_id', sourceAccountId);
...
export async function reconnectSourceAccount(sourceAccountId: string, orderItemId: string, accountId: string): Promise<void> {
  ...
  const { item } = await loadOrderItemForAccount(orderItemId, accountId);
  if (item.assigned_source_account_id !== null) {
    throw new Error("Order item is already connected to a source account");
  }
