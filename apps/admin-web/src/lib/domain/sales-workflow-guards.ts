...
export function ensureOrderRenewalAllowed(status: OrderStatus | string | null | undefined): void {
  ...
  if (!ALLOWED_ORDER_RENEWAL_STATUSES.has(normalizedStatus)) {
    throw new UnprocessableEntityError(
      "Không thể gia hạn đơn hàng. Chỉ có thể gia hạn đơn hàng ở trạng thái active hoặc expired.",
    );
  }
}
...
export function ensureOrderRefundRequestAllowed(
  ...
  if ((order.status ?? "") === "refunded") {
    throw new ConflictError("Đơn hàng đã hoàn tiền, không thể tạo yêu cầu mới.");
  }

  if (Math.max(0, Number(order.total_paid ?? 0)) <= 0) {
    throw new UnprocessableEntityError("Đơn hàng chưa có thanh toán để hoàn tiền.");
  }
}
