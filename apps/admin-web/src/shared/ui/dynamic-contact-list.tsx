...
const contactTypes = [
  { value: "phone", label: vi.customers.dynamicContactList.types.phone, icon: Phone },
  { value: "email", label: vi.customers.dynamicContactList.types.email, icon: Mail },
  { value: "zalo", label: vi.customers.dynamicContactList.types.zalo, icon: MessageCircle },
  { value: "facebook", label: vi.customers.dynamicContactList.types.facebook, icon: MessageCircle },
  { value: "telegram", label: vi.customers.dynamicContactList.types.telegram, icon: Send },
  { value: "other", label: vi.customers.dynamicContactList.types.other, icon: Phone },
] as const;
...
export function DynamicContactList({
  ...
  description = vi.customers.dynamicContactList.description,
  onFacebookResolved,
    ...
    <div className="space-y-4">
      <div className="border-b border-[var(--border-soft)] pb-2 mb-4 flex items-center justify-between">
        ...
        <Button variant="secondary" size="sm" onClick={addContact} type="button" className="h-8 gap-1.5 text-[var(--accent)] hover:text-[var(--accent-strong)]">
          <Plus className="size-3.5" /> {vi.customers.dynamicContactList.addChannel}
        </Button>
      </div>
    ...
    fetch(`/api/proxy/facebook-id?link=${encodeURIComponent(debouncedValue.trim())}`)
      ...
      .then((data: { id?: string; name?: string; profileUrl?: string; error?: string }) => {
        ...
        if (data.error || !data.id) {
          setFbState({ loading: false, error: data.error ?? vi.customers.dynamicContactList.noFacebookId });
          return;
        }
        setFbState({
          loading: false,
      ...
      .catch(() => {
        if (!cancelled) setFbState({ loading: false, error: vi.customers.dynamicContactList.connectError });
      });
    ...
    <div className="flex flex-col gap-2 p-3 rounded-xl border border-[var(--border-soft)] bg-[var(--surface-light)]">
      <div className="flex gap-2 items-start lg:items-center flex-col lg:flex-row">
        <div className="flex-1 w-full flex flex-col sm:flex-row gap-2">
          ...
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={
                contact.type === "phone" ? vi.customers.dynamicContactList.placeholders.phone
                : contact.type === "email" ? vi.customers.dynamicContactList.placeholders.email
                : contact.type === "facebook" ? vi.customers.dynamicContactList.placeholders.facebook
                : vi.customers.dynamicContactList.placeholders.other
              }
              required
            />
        ...
        <div className="flex items-center gap-2 self-start lg:self-auto flex-wrap">
          {isFacebook && (
            <Button variant="secondary" size="sm" type="button" className="h-10 px-3 text-[12px] whitespace-nowrap gap-1.5" onClick={() => appToast.info(vi.customers.dynamicContactList.verifyDescription, { description: vi.customers.dynamicContactList.verifyHint })}>
              <CheckCircle className="size-3.5 text-blue-500" />
              {vi.customers.dynamicContactList.verify}
            </Button>
          )}
          ...
          <label className="flex items-center gap-1.5 cursor-pointer h-10 px-3 border border-[var(--border-soft)] rounded-lg hover:bg-[var(--bg-app)]">
            ...
            />
            <span className="text-[11px] font-medium text-[var(--fg-muted)] whitespace-nowrap">{vi.customers.dynamicContactList.primary}</span>
          </label>
      ...
      {isFacebook && fbState.id && (
        <div className="flex items-center gap-2 px-2 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-lg text-[12px]">
          ...
          {fbState.profileUrl && (
              ...
              {vi.customers.dynamicContactList.openFacebook}
              </a>
          )}
      </div>
      )}
