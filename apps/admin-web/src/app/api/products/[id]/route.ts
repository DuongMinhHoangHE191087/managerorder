...
// BUG #2 FIX: PUT → PATCH (convention fix) + requirePermissions guard
// BUG #7 FIX: renamed from PUT to PATCH for partial update semantics
...
// Keep PUT as alias for backward compatibility
export const PUT = PATCH;

export const DELETE = withErrorHandler(
  withAccount(
    requirePermissions<{ id: string }>(["order:delete"])(async (_request: NextRequest, { accountId, params }) => {
      const { id } = await params;
