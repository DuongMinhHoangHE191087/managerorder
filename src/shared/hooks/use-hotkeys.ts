  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "n" && !e.ctrlKey && !e.metaKey) {
        ...
        showShortcutToast("Đã dùng phím tắt: N (Tạo đơn mới)");
      }
      ...
      if (e.key === "o" && !e.ctrlKey && !e.metaKey) {
        e.preventDefault();
        router.push("/orders");
        showShortcutToast("Đã dùng phím tắt: O (Danh sách đơn hàng)");
      }
      ...
      if (e.key === "i" && !e.ctrlKey && !e.metaKey) {
        ...
        showShortcutToast("Đã dùng phím tắt: I (Quản lý kho)");
      }
    };
