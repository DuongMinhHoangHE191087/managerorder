...
export function NotificationsDrawer({
  ...
  const notificationLabel = useMemo(() => {
    if (notifications.length === 0) {
      return "Không có tín hiệu nào cần xử lý";
    }

    return `${notifications.length} mục cần theo dõi`;
            ...
            <div className="empty-state min-h-[220px]">
              ...
              <div>
                <p className="text-sm font-bold text-[var(--fg-base)]">
                  Chưa có vấn đề nào cần thao tác.
                </p>
                <p className="text-sm text-[var(--fg-muted)]">
                  Drawer này chỉ hiện các tín hiệu hành động được.
                </p>
              </div>
            </div>
          ) : (
