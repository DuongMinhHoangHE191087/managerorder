...
import { BarChart3, Clock, TrendingUp, AlertTriangle, Users } from "lucide-react";
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SectionCard title="📊 Request Volume (24h)" description="Lượng request và error theo giờ">
        ...
        <SectionCard title="🔥 Top Endpoints" description="Endpoints có lượng truy cập cao nhất">
        ...
        <SectionCard
          title="🛡️ Rate Limit Hits"
          description="Endpoints bị chặn bởi rate limiter"
          action={
            <span className="flex items-center gap-1.5 text-[11px] font-bold text-amber-600">
              {metrics.rateLimits.totalHits} hits | {metrics.rateLimits.blocked} blocked
            </span>
          }
        >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2 mt-1.5">
                    <div
                      ...
                      style={{ width: `${(ep.hits / metrics.rateLimits.topEndpoints[0].hits) * 100}%` }}
                    />
                  </div>
                </div>
        ...
        <SectionCard title="🔐 Authentication" description="Login, token refresh, và failure stats">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: "Login Success", value: metrics.authMetrics.loginSuccess, color: "text-emerald-600" },
              { label: "Login Failed", value: metrics.authMetrics.loginFailed, color: "text-red-500" },
              { label: "Token Refreshed", value: metrics.authMetrics.tokenRefreshed, color: "text-blue-600" },
              { label: "Active Users", value: metrics.authMetrics.activeUsers, color: "text-violet-600" },
            ].map((item) => (
        ...
        <SectionCard title="🚨 Recent Errors" description="Lỗi API gần nhất (5 phút)">
          <div className="space-y-2">
