...
import { Box, CheckCircle2, ChevronLeft, ChevronRight, Clock, Search } from "lucide-react";
...
export const getExpiryBadge = (expiresAt: string) => {
  ...
  if (diff < 0) return { label: "Da het han", class: "bg-[var(--danger)]/10 text-[var(--danger)] border-[var(--danger)]/20" };
  if (diff <= 7) return { label: `Con ${diff} ngay`, class: "bg-[var(--warning)]/10 text-[var(--warning)] border-[var(--warning)]/20" };
...
export const OrdersTable = React.memo(function OrdersTable({
  ...
  return (
    <section className="app-card overflow-hidden border border-[var(--border-soft)] bg-[rgba(255,255,255,0.94)] shadow-[0_18px_44px_rgba(15,23,42,0.05)] transition-all hover:shadow-[0_22px_52px_rgba(15,23,42,0.08)]">
      <div className="flex items-center justify-between gap-4 border-b border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,250,244,0.84))] px-5 py-4">
        ...
        <div className="min-w-0">
          <h3 className="text-[15px] font-bold tracking-tight text-[var(--fg-base)]">Don hang</h3>
          <p className="text-[12px] text-[var(--fg-muted)]">Quan ly don, thanh toan, cap phat va trang thai trong mot dong.</p>
        </div>

        <span className="whitespace-nowrap text-[12px] font-medium text-[var(--fg-muted)]">
          Tong cong <strong className="text-[var(--accent)]">{totalElements}</strong> don
        </span>
      </div>
      ...
      <div className="p-4 sm:p-5">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-[1.5rem] border border-dashed border-[var(--border-soft)] bg-[rgba(255,255,255,0.84)] py-20 text-center shadow-sm">
            ...
            <h3 className="text-lg font-bold text-[var(--fg-base)]">Khong tim thay don hang</h3>
            <p className="mt-1 text-[14px] text-[var(--fg-muted)]">Thu thay doi bo loc hoac tao don hang moi.</p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4">
                    <button
                      type="button"
                      onClick={(e) => toggleOne(e, order.id)}
                      className={cn(
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-1.5 flex flex-wrap items-center gap-2">
                      <span className="rounded border border-[var(--accent)]/20 bg-[var(--accent)]/10 px-2 py-0.5 font-mono text-[13px] font-bold text-[var(--accent)]">
                  ...
                  <div className="w-full rounded-[1.15rem] border border-[var(--border-soft)] bg-[rgba(246,250,244,0.72)] p-3.5 transition-colors group-hover:border-[var(--accent)]/20 group-hover:bg-white md:w-[240px]">
                    <div className="space-y-1.5">
                      <div className="mb-1 flex items-end justify-between">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--fg-muted)]">Tien trinh</span>
                    ...
                    <div className="mt-3 space-y-1.5 border-t border-[var(--border-soft)] pt-3">
                      <div className="flex items-end justify-between">
                        ...
                        <div className="text-right">
                          <span className="block text-[10px] font-medium text-[var(--fg-muted)]">Lai/Lo</span>
                        ...
                        <div className="mt-0.5 flex items-center gap-2">
                          ...
                          {total > paid ? (
                            <span className="whitespace-nowrap text-[10px] font-bold text-[var(--danger)]">
                              Cong no {formatMoney(order.balance_due_vnd ?? total - paid)}
                            </span>
                          ) : (
                            <span className="whitespace-nowrap text-[10px] font-bold text-emerald-500">Du TT</span>
                          )}
                        </div>
                      ) : null}
                    </div>
      ...
      {totalElements > 0 || isLoading ? (
        <div className="flex items-center justify-between gap-4 border-t border-[var(--border-soft)] bg-[rgba(255,255,255,0.88)] px-6 py-3.5">
          <div className="flex items-center gap-3 overflow-x-auto pb-1 sm:pb-0">
            <span className="whitespace-nowrap text-[12px] font-bold uppercase tracking-wide text-[var(--fg-muted)]">Moi trang:</span>
            ...
            <span className="hidden whitespace-nowrap border-l border-[var(--border-soft)] pl-3 text-[12px] font-medium text-[var(--fg-muted)] lg:inline">
              Ket qua: <strong className="text-[var(--fg-base)]">{(pageIndex * pageSize) + 1}â€“{Math.min((pageIndex + 1) * pageSize, totalElements)}</strong> / {totalElements}
            </span>
          </div>
