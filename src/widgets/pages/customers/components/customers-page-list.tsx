...
function getDebtBadge(amount: number, overdueDays: number) {
  if (amount <= 0) return { label: "AN TOÀN", class: "bg-[var(--accent)]/10 text-[var(--accent)]" };
  if (overdueDays > 30) return { label: "QUÁ HẠN", class: "bg-[var(--danger)]/10 text-[var(--danger)]" };
  if (overdueDays > 7) return { label: "SẮP HẠN", class: "bg-[var(--warning)]/10 text-[var(--warning)]" };
  return { label: "CÓ NỢ", class: "bg-[var(--warning)]/10 text-[var(--warning)]" };
}
      ...
      <div className="mb-4 flex items-center justify-between rounded-[1.2rem] border border-[var(--border-soft)] bg-[rgba(246,250,244,0.78)] p-3 shadow-sm">
        <label className="group flex cursor-pointer items-center gap-3 px-2">
          ...
          <span className="select-none text-[13px] font-bold text-[var(--fg-base)]">
            {selectedCount > 0 ? `Đã chọn ${selectedCount} khách hàng` : "Chọn tất cả"}
          </span>
        </label>
        <span className="text-[12px] font-medium text-[var(--fg-muted)]">
          Tổng cộng <strong className="text-[var(--accent)]">{totalElements}</strong> khách hàng
        </span>
      </div>
      ...
      <div className="flex min-h-[400px] flex-col gap-3">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <h3 className="text-[15px] font-bold text-[var(--fg-base)]">Không có khách hàng nào</h3>
            {debouncedQuery ? (
              <p className="mt-1 text-[13px] text-[var(--fg-muted)]">Thử tìm kiếm khác</p>
            ) : (
              ...
              >
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  <div className="mb-1.5 flex flex-wrap items-center gap-2">
                    {customer.segment && <RfmBadge segment={customer.segment} rfmScore={customer.rfmScore} showScore size="sm" />}
                    <span
                  ...
                  <div className="mt-1.5 flex items-center gap-2 text-[13px] text-[var(--fg-muted)]">
                    ...
                    {(customer.tags ?? []).length > 0 ? <span className="h-1 w-1 rounded-full bg-gray-300" /> : null}
                ...
                <div className="absolute right-4 top-4 z-10" onClick={(event) => event.stopPropagation()}>
                  <ActionMenu
                    items={[
                      { label: "Xem chi tiết", icon: <Eye className="size-4" />, onClick: () => onOpenCustomer(customer.id) },
                      { label: "Chỉnh sửa", icon: <Pencil className="size-4" />, onClick: () => onEditCustomer(customer) },
                      { label: "Cập nhật công nợ", icon: <DollarSign className="size-4" />, onClick: () => onRenewCustomer(customer) },
                      ...(customer.debtAmountVnd > 0
                        ? [{ label: "Đã thu toàn bộ nợ", icon: <DollarSign className="size-4" />, onClick: () => onClearDebt(customer.id) }]
                        : []),
                      { label: "Xóa khách hàng", icon: <Trash2 className="size-4" />, onClick: () => onDeleteCustomer(customer), variant: "danger", dividerBefore: true },
                    ]}
                  />
                </div>
        ...
        <div className="mt-6 flex flex-col items-center justify-between gap-4 rounded-[1.2rem] border border-[var(--border-soft)] bg-[rgba(255,255,255,0.88)] p-4 sm:flex-row">
          <div className="flex w-full items-center gap-3 overflow-x-auto pb-1 sm:w-auto sm:pb-0">
            ...
            <span className="hidden whitespace-nowrap border-l border-[var(--border-soft)] pl-3 text-[12px] font-medium text-[var(--fg-muted)] lg:inline">
              K.quả: <strong className="text-[var(--fg-base)]">{pageIndex * pageSize + 1}–{Math.min((pageIndex + 1) * pageSize, totalElements)}</strong> / {totalElements}
            </span>
          </div>
