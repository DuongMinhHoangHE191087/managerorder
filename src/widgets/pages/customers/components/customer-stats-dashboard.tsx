...
export const CustomerStatsDashboard = React.memo(function CustomerStatsDashboard({ customers, groups }: CustomerStatsDashboardProps) {
  const stats = useMemo(() => {
    ...
    const typeBars: StatBar[] = [
      { label: "Khách lẻ", value: byType.retail, color: "#6366f1", percent: total ? (byType.retail / total) * 100 : 0 },
      { label: "Bán sỉ", value: byType.wholesale, color: "#f59e0b", percent: total ? (byType.wholesale / total) * 100 : 0 },
      { label: "Đại lý", value: byType.agency, color: "#10b981", percent: total ? (byType.agency / total) * 100 : 0 },
    ];
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in slide-in-from-top-2 duration-300">
      ...
      <div className="bg-white border border-[var(--border-soft)] rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h4 className="text-[13px] font-bold text-[var(--fg-base)]">Phân bố loại KH</h4>
        </div>
      ...
      <div className="bg-white border border-[var(--border-soft)] rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h4 className="text-[13px] font-bold text-[var(--fg-base)]">Tổng quan công nợ</h4>
        </div>
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="text-[12px] text-[var(--fg-muted)]">Tổng nợ</span>
            <span className="text-[14px] font-black text-[var(--danger)]">{formatMoney(stats.totalDebt)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-[12px] text-[var(--fg-muted)]">KH đang nợ</span>
            <span className="text-[14px] font-bold text-[var(--fg-base)]">{stats.withDebt}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-[12px] text-[var(--fg-muted)]">Công nợ TB/KH</span>
          ...
          <div className="flex justify-between items-center">
            <span className="text-[12px] text-[var(--fg-muted)] flex items-center gap-1">
              <AlertTriangle className="size-3 text-[var(--danger)]" />Quá hạn (&gt;7d)
            </span>
      ...
      <div className="bg-white border border-[var(--border-soft)] rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h4 className="text-[13px] font-bold text-[var(--fg-base)]">Top 5 KH nợ nhiều</h4>
        </div>
        {stats.topDebtors.length === 0 ? (
          <p className="text-[12px] text-[var(--fg-muted)] text-center py-4">Không có KH nào đang nợ 🎉</p>
        ) : (
      ...
      {groups.length > 0 && (
        <div className="bg-white border border-[var(--border-soft)] rounded-2xl p-5 md:col-span-2 lg:col-span-3">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-[13px] font-bold text-[var(--fg-base)]">Phân bố theo nhóm</h4>
          </div>
