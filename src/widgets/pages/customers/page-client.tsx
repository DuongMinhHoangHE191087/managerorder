...
export default function CustomersPage() {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      setDeletingCustomer(null);
      appToast.success("Đã xóa khách hàng!");
    } catch {
      appToast.error("Lỗi xóa khách hàng");
    }
  ...
  const handleBatchDeleteConfirm = useCallback(async () => {
    try {
      ...
      appToast.success(`Đã xóa ${result.deletedCount} khách hàng`);
      setSelectedIds(new Set());
      setShowBatchDeleteConfirm(false);
      setBatchDepInfo(null);
    } catch {
      appToast.error("Lỗi xóa hàng loạt");
    }
  ...
  const handleBatchTypeConfirm = useCallback(async (tier: "retail" | "wholesale" | "agency") => {
    try {
      ...
      appToast.success(`Đã cập nhật ${result.updatedCount} khách hàng`);
      setSelectedIds(new Set());
      setShowBatchTierModal(false);
    } catch {
      appToast.error("Lỗi cập nhật hàng loạt");
    }
  ...
  const handleClearDebt = useCallback(async (id: string) => {
    try {
      ...
      appToast.success("Đã thanh toán toàn bộ công nợ!");
    } catch {
      appToast.error("Lỗi thanh toán nợ");
    }
  }, [updateCustomer]);
  ...
  const handleRecalculateRfm = useCallback(async () => {
    try {
      ...
      appToast.success(`Đã cập nhật RFM cho ${result.updatedCount} khách hàng!`);
    } catch {
      appToast.error("Lỗi khi tính lại RFM");
    }
  }, [recalculateRfm]);
  ...
  const handleRenewal = useCallback(async (debtDays: string, debtAmount: string) => {
    ...
    setRenewingCustomer(null);
    appToast.success("Đã cập nhật tình trạng công nợ!");
      ...
      <PageContainer className="relative">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          >
            ...
            {showGroupTag ? "Ẩn Nhóm & Tag" : "Quản lý Nhóm & Tag"}
          </button>
