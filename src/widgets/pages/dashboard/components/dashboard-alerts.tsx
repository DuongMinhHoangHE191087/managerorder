...
const QUICK_ACTIONS = [
  { href: "/orders/new", label: "Tạo Đơn", icon: "add_shopping_cart", color: "bg-[var(--accent)]/10 text-[var(--accent)] hover:bg-[var(--accent)] hover:text-white" },
  { href: "/customers", label: "Khách hàng", icon: "person_add", color: "bg-blue-50/80 text-blue-600 hover:bg-blue-600 hover:text-white" },
  { href: "/products", label: "Sản phẩm", icon: "inventory_2", color: "bg-amber-50/80 text-amber-600 hover:bg-amber-600 hover:text-white" },
  { href: "/inventory", label: "Kho hàng", icon: "warehouse", color: "bg-purple-50/80 text-purple-600 hover:bg-purple-600 hover:text-white" },
] as const;
      ...
      <div className="app-card flex flex-col overflow-hidden border border-[var(--border-soft)] bg-[rgba(255,255,255,0.94)] shadow-[0_16px_38px_rgba(15,23,42,0.05)]">
        <div className="flex items-center justify-between border-b border-[var(--border-soft)] px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-[13px] font-bold text-[var(--fg-base)]">{pendingOrders.length} đơn chờ thanh toán</p>
          </div>
      ...
      <div className="app-card flex flex-col overflow-hidden border border-[var(--border-soft)] bg-[rgba(255,255,255,0.94)] shadow-[0_16px_38px_rgba(15,23,42,0.05)]">
        <div className="flex items-center justify-between border-b border-[var(--border-soft)] px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-[13px] font-bold text-[var(--fg-base)]">Top 5 khách hàng nợ cước</p>
          </div>
            ...
            >
              <div>
                ...
                <p className="mt-0.5 text-[11px] font-medium text-[var(--danger)]">
                  Trễ {customer.debtOverdueDays} ngày
                </p>
              </div>
      ...
      <div className="app-card flex flex-col overflow-hidden border border-[var(--border-soft)] bg-[rgba(255,255,255,0.94)] shadow-[0_16px_38px_rgba(15,23,42,0.05)]">
        <div className="flex items-center justify-between border-b border-[var(--border-soft)] px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-[13px] font-bold text-[var(--fg-base)]">Top 5 Nguồn (&lt;7 ngày)</p>
          </div>
        ...
        <div className="flex-1 divide-y divide-[var(--border-soft)] overflow-y-auto">
          {expiringAccounts.map((account) => (
            ...
            >
              ...
              <div className="text-right">
                <p className="text-[12px] font-black text-[var(--danger)]">{account.daysLeft} ngày</p>
              </div>
            </div>
          ))}
