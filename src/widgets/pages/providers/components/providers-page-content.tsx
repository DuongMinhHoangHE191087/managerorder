...
export default function ProvidersPage() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      appToast.success("Đã xóa nhà cung cấp!");
    } catch {
      appToast.error("Lỗi xóa nhà cung cấp");
    }
  }
    ...
    <AppLayout>
      ...
      <PageContainer className="relative">
        <div className="app-card flex flex-col gap-4 border border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,250,244,0.84))] px-5 py-5 shadow-[0_18px_44px_rgba(15,23,42,0.05)] md:flex-row md:items-end md:justify-between mb-2 mt-2">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-[var(--fg-base)]">Nhà cung cấp</h1>
            <p className="text-[15px] text-[var(--fg-muted)] font-medium tracking-wide">Quản lý mạng lưới đối tác, nguồn cung dịch vụ & sản phẩm.</p>
          </div>
          <button
        ...
        <div className="grid gap-6 md:grid-cols-2 mb-8">
          <div>
            <div className="app-card flex h-full flex-col gap-2 p-6">
              <div className="flex justify-between items-start">
                <p className="text-[var(--fg-muted)] text-[11px] font-bold uppercase tracking-wider">Tổng nhà cung cấp</p>
          ...
          <div>
            <div className="app-card flex h-full flex-col gap-2 p-6">
              <div className="flex justify-between items-start">
                <p className="text-[var(--fg-muted)] text-[11px] font-bold uppercase tracking-wider">Đối tác VIP</p>
        ...
        <div className="w-full flex flex-col relative py-4 min-h-0 items-stretch space-y-3">
          {totalProviders === 0 ? (
            <div className="p-12 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-[15px] font-bold text-[var(--fg-base)]">{searchQuery ? 'Không tìm thấy nhà cung cấp nào' : 'Chưa có nhà cung cấp nào'}</h3>
              {!searchQuery && (
            ...
            paginatedProviders.map((provider) => {
              ...
              const actionMenuItems = [
                { label: "Xem chi tiết", icon: <Eye className="size-4" />, onClick: () => router.push(`/providers/${provider.id}`) },
                { label: "Chỉnh sửa nhanh", icon: <Pencil className="size-4" />, onClick: () => setEditingProvider(provider) },
                { label: "Cập nhật tín nhiệm", icon: <Star className="size-4" />, onClick: () => setEditingProvider(provider) },
                { 
                            ...
                            <div className="flex flex-col gap-1.5 mt-2">
                              ...
                              <div className="flex items-center gap-2 mt-1 w-fit">
                                <div className="flex items-center gap-1.5 text-[12px] font-medium text-[var(--accent)] bg-[var(--accent)]/10 border border-[var(--accent)]/20 px-2 py-1 rounded-md">
                                   ...
                                   <span><strong className="font-black">{provider.purchaseOrderCount || 0}</strong> đơn nhập</span>
                                </div>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="w-full md:w-[260px] lg:w-[280px] shrink-0 flex flex-col justify-center gap-3 bg-[var(--surface-light)] p-3.5 md:p-4 rounded-xl border border-[var(--border-soft)] group-hover:bg-white group-hover:border-[var(--accent)]/30 group-hover:shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)] transition-all duration-300">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] md:text-[11px] font-bold text-[var(--fg-muted)] uppercase tracking-wider flex items-center gap-1.5">
                          <Star className="size-3.5 fill-amber-500 text-amber-500" /> Tín Nhiệm
                        </span>
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] md:text-[11px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">Tổng nhập</span>
                        ...
                        <div className="flex justify-between items-center rounded-lg p-2 md:p-2.5 -mx-2 bg-red-50/50 border border-red-100">
                          <span className="text-[10px] md:text-[11px] font-bold text-red-500 uppercase tracking-wider">Công nợ</span>
          ...
          {totalProviders > 0 && (
            <div className="mt-6 flex flex-col items-center justify-between gap-4 rounded-[1.2rem] border border-[var(--border-soft)] bg-[rgba(255,255,255,0.88)] p-4 sm:flex-row">
              <div className="flex items-center gap-3 w-full sm:w-auto overflow-x-auto custom-scrollbar pb-1 sm:pb-0">
                <span className="text-[12px] text-[var(--fg-muted)] font-bold tracking-wide uppercase whitespace-nowrap">Mỗi trang:</span>
                ...
                <span className="text-[12px] text-[var(--fg-muted)] font-medium whitespace-nowrap hidden lg:inline border-l border-[var(--border-soft)] pl-3">
                  K.quả: <strong className="text-[var(--fg-base)]">{(pageIndex * pageSize) + 1}–{Math.min((pageIndex + 1) * pageSize, totalProviders)}</strong> / {totalProviders}
                </span>
              </div>
      ...
      {/* ===== DELETE CONFIRMATION ===== */}
      <Modal isOpen={!!deletingProvider} onClose={() => setDeletingProvider(null)} title="Xác nhận xóa" size="sm"
        footer={
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setDeletingProvider(null)}>Hủy</Button>
      ...
      >
        <div className="text-center py-4">
          ...
          <p className="text-[13px] text-[var(--fg-muted)]">
            Nhà cung cấp <span className="font-bold text-[var(--fg-base)]">&ldquo;{deletingProvider?.name}&rdquo;</span> sẽ bị xóa vĩnh viễn.
          </p>
        </div>
      </Modal>
