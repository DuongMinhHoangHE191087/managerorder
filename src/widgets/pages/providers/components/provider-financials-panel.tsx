...
export function ProviderFinancialsPanel({
  ...
  return (
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
      <div className="app-card overflow-hidden border border-[var(--border-soft)] bg-[rgba(255,255,255,0.94)] shadow-[0_16px_38px_rgba(15,23,42,0.05)]">
        <div className="border-b border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,250,244,0.84))] px-5 py-4">
          ...
          <p className="mt-1 text-[11px] text-[var(--fg-muted)]">
            Tóm lược công nợ, số tiền đã thanh toán và tiến độ xử lý.
          </p>
        </div>
        <div className="grid grid-cols-2 gap-3 p-5">
          <div className="rounded-[1.15rem] border border-[var(--border-soft)] bg-[var(--surface-light)] p-4">
            <p className="mb-1 text-[10px] font-bold uppercase text-[var(--fg-muted)]">Tổng mua</p>
            <p className="text-xl font-black text-[var(--fg-base)]">{formatMoney(stats.totalPurchases)}</p>
          </div>
          <div className="rounded-[1.15rem] border border-[var(--border-soft)] bg-[var(--surface-light)] p-4">
            <p className="mb-1 text-[10px] font-bold uppercase text-[var(--fg-muted)]">Đã thanh toán</p>
            <p className="text-xl font-black text-emerald-500">{formatMoney(stats.totalPaid)}</p>
          </div>
          <div className="rounded-[1.15rem] border border-[var(--border-soft)] bg-[var(--surface-light)] p-4">
            <p className="mb-1 text-[10px] font-bold uppercase text-[var(--fg-muted)]">Công nợ</p>
            <p className="text-xl font-black text-red-500">{formatMoney(stats.totalDebt)}</p>
          </div>
          <div className="rounded-[1.15rem] border border-[var(--border-soft)] bg-[var(--surface-light)] p-4">
            <p className="mb-1 text-[10px] font-bold uppercase text-[var(--fg-muted)]">Hoàn thành</p>
            <p className="text-xl font-black text-[var(--accent)]">{stats.completedPOs}/{purchaseOrders.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="app-card overflow-hidden border border-[var(--border-soft)] bg-[rgba(255,255,255,0.94)] shadow-[0_16px_38px_rgba(15,23,42,0.05)]">
        <div className="border-b border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,250,244,0.84))] px-5 py-4">
          <h3 className="text-[15px] font-bold tracking-tight text-[var(--fg-base)]">Hoạt động gần đây</h3>
        </div>
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-[13px] font-bold text-[var(--fg-base)]">
                  {getPurchaseOrderNames(order) || "Đơn nhập hàng"}
                </p>
              ...
              <div className="text-right">
                ...
                <p className="text-[11px] font-bold text-emerald-500">TT: {formatMoney(order.total_paid)}</p>
              </div>
            </div>
          ))}
          {purchaseOrders.length === 0 ? (
            <p className="text-[13px] text-[var(--fg-muted)]">Chưa có giao dịch nào.</p>
          ) : null}
        </div>
      </div>
    </div>
