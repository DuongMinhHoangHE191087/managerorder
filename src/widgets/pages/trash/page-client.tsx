...
const TABS: TabConfig[] = [
  { key: "customers", label: "Khách hàng", icon: Users, gradient: "from-blue-500/20 to-blue-600/10" },
  { key: "orders", label: "Đơn hàng", icon: ShoppingCart, gradient: "from-green-500/20 to-green-600/10" },
  { key: "products", label: "Sản phẩm", icon: Package, gradient: "from-purple-500/20 to-purple-600/10" },
  { key: "providers", label: "Nhà cung cấp", icon: Truck, gradient: "from-orange-500/20 to-orange-600/10" },
  { key: "source_accounts", label: "Tài khoản nguồn", icon: Mail, gradient: "from-cyan-500/20 to-cyan-600/10" },
  { key: "license_keys", label: "Kho", icon: Key, gradient: "from-amber-500/20 to-amber-600/10" },
  { key: "short_links", label: "Link Rút Gọn", icon: LinkIcon, gradient: "from-pink-500/20 to-pink-600/10" },
];
...
const DISPLAY_FIELDS: Record<EntityType, { key: string; label: string }[]> = {
  ...
  orders: [
    ...
    { key: "total_amount_vnd", label: "Tổng tiền" },
  ],
  products: [
  ...
  source_accounts: [
    ...
    { key: "provider", label: "Nhà CC" },
  ],
  license_keys: [
    { key: "key_code", label: "Mã key" },
    { key: "status", label: "Trạng thái" },
  ],
  short_links: [
    { key: "slug", label: "Slug" },
    { key: "target_url", label: "Đích" },
    { key: "current_clicks", label: "Click" },
  ],
};
...
const DETAIL_FIELDS: Record<EntityType, { key: string; label: string }[]> = {
  ...
  products: [
    { key: "price_vnd", label: "Giá bán" },
    { key: "cost_vnd", label: "Giá vốn" },
  ...
  license_keys: [
    { key: "product_id", label: "Sản phẩm ID" },
    { key: "notes", label: "Ghi chú" },
    { key: "created_at", label: "Ngày tạo" },
  ],
  short_links: [
    { key: "title", label: "Tiêu đề" },
    { key: "expires_at", label: "Ngày hết hạn" },
    { key: "created_at", label: "Ngày tạo" },
  ],
};
...
export default function TrashPage() {
  ...
  function handlePurge() {
    if (purgeConfirmText !== "XÓA VĨNH VIỄN") return;
    purgeMut.mutate(
      { type: activeTab, ids: Array.from(selectedIds) },
      {
        onSuccess: () => {
    ...
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--fg-base)]">Thùng rác</h1>
            <p className="text-sm text-[var(--fg-muted)]">
              {totalTrash > 0 ? `${totalTrash} mục đã xóa` : "Không có mục nào trong thùng rác"}
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input
            ...
            placeholder={`Tìm trong ${activeTabConfig.label.toLowerCase()}...`}
      ...
      <div className="glass-card !rounded-2xl overflow-hidden">
        {isLoading ? (
          <div className="p-16 text-center">
            ...
            <p className="text-sm text-[var(--fg-muted)]">Đang tải dữ liệu...</p>
          </div>
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-sm text-[var(--fg-muted)] max-w-[260px] mx-auto">
              {searchQuery
                ? `Không có ${activeTabConfig.label.toLowerCase()} nào khớp với "${searchQuery}"`
                : `Không có ${activeTabConfig.label.toLowerCase()} nào đã xóa`}
            </p>
          </div>
        ) : (
                  ...
                  <div key={id} className="flex flex-col">
                    ...
                    >
                      ...
                      <div className="col-span-1 lg:col-span-2 flex flex-col mt-2 lg:mt-0 pt-3 lg:pt-0 border-t border-[var(--border-soft)]/50 lg:border-none">
                        <span className="lg:hidden text-[11px] uppercase tracking-widest text-[var(--fg-muted)] block mb-1 font-bold">Thời gian xóa</span>
                        ...
                        <div className="p-4 sm:px-6 pl-4 lg:pl-[104px]">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            <div className="bg-red-500/5 rounded-xl p-3 border border-red-500/10 shadow-sm">
                              <p className="text-[10px] font-bold text-red-500 uppercase tracking-wider mb-1">Đã xóa lúc</p>
                              ...
                              {Boolean(item.deleted_by) && (
                                <div className="mt-2 pt-2 border-t border-red-500/10">
                                  <p className="text-[10px] font-bold text-red-500 uppercase tracking-wider mb-0.5">Người xóa</p>
        ...
        {!isLoading && totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-[var(--border-soft)]">
            <span className="text-sm text-[var(--fg-muted)]">
              Hiển thị <span className="font-semibold text-[var(--fg-base)]">{(currentPage - 1) * itemsPerPage + 1}</span> đến <span className="font-semibold text-[var(--fg-base)]">{Math.min(currentPage * itemsPerPage, filteredItems.length)}</span> trong số <span className="font-semibold text-[var(--fg-base)]">{filteredItems.length}</span> mục
            </span>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-full max-w-xl px-4">
          <div className="glass-card-strong !rounded-2xl px-5 py-3.5 flex items-center gap-3 !shadow-[0_8px_40px_rgba(0,0,0,0.15)]">
            <div className="flex items-center gap-2 mr-auto">
              ...
              <span className="text-sm font-medium text-[var(--fg-base)]">đã chọn</span>
            </div>
            <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          ...
          >
            ...
            <p className="text-sm text-[var(--fg-muted)] text-center mb-5 leading-relaxed">
              Hành động này <strong className="text-[var(--danger)]">KHÔNG THỂ HOÀN TÁC</strong>.
              <br />
            ...
            <label className="block text-xs font-semibold text-[var(--fg-muted)] uppercase tracking-wider mb-2">
              Nhập <span className="text-[var(--danger)]">XÓA VĨNH VIỄN</span> để xác nhận
            </label>
            <input
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handlePurge}
                disabled={purgeConfirmText !== "XÓA VĨNH VIỄN" || purgeMut.isPending}
              ...
              >
                {purgeMut.isPending ? (
                  ...
                  `Xóa ${selectedIds.size} mục vĩnh viễn`
                )}
              </button>
            </div>
          </div>
