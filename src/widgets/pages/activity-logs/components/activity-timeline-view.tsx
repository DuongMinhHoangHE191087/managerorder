...
function getActionConfig(actionType: string) {
  switch (actionType) {
    case "CUSTOMER_CREATED":
      return { icon: User, color: "text-emerald-400", bg: "bg-emerald-500/20", label: "Tạo khách hàng" };
    case "ORDER_CREATED":
      return { icon: Plus, color: "text-indigo-400", bg: "bg-indigo-500/20", label: "Tạo đơn hàng mới" };
    case "ORDER_UPDATED":
      return { icon: RefreshCw, color: "text-blue-400", bg: "bg-blue-500/20", label: "Cập nhật trạng thái đơn" };
    case "ORDER_DELETED":
      return { icon: Trash2, color: "text-red-400", bg: "bg-red-500/20", label: "Xóa đơn hàng" };
    case "PAYMENT_ADDED":
      return { icon: CreditCard, color: "text-emerald-400", bg: "bg-emerald-500/20", label: "Thanh toán / Gia hạn" };
    case "INVENTORY_ASSIGNED":
      return { icon: Package, color: "text-purple-400", bg: "bg-purple-500/20", label: "Gán kho vào đơn" };
    case "ALLOCATION_CONFIRMED":
      return { icon: PlayCircle, color: "text-purple-400", bg: "bg-purple-500/20", label: "Cấp phát kho" };
    case "ALLOCATION_RELEASED":
      return { icon: AlertTriangle, color: "text-amber-400", bg: "bg-amber-500/20", label: "Giải phóng kho" };
    case "RESERVED_NICK_ADDED":
      return { icon: Plus, color: "text-violet-400", bg: "bg-violet-500/20", label: "Thêm nick đặt trước" };
    case "RESERVED_NICK_REMOVED":
      return { icon: Trash2, color: "text-rose-400", bg: "bg-rose-500/20", label: "Xóa nick đặt trước" };
    case "INVENTORY_KEY_CREATED":
      return { icon: Edit3, color: "text-cyan-400", bg: "bg-cyan-500/20", label: "Tạo license key" };
    case "SLOTS_RECALCULATED":
      return { icon: RefreshCw, color: "text-blue-400", bg: "bg-blue-500/20", label: "Đồng bộ lại slot" };
    case "WARRANTY_REASSIGNED":
      return { icon: AlertTriangle, color: "text-amber-400", bg: "bg-amber-500/20", label: "Bảo hành / Đổi kho" };
    case "INVENTORY_STATUS_CHANGED":
      return { icon: CheckCircle, color: "text-slate-400", bg: "bg-slate-500/20", label: "Cập nhật trạng thái kho" };
    case "PREMIUM_MIGRATION_REQUEST_CREATED":
      return { icon: ArrowLeftRight, color: "text-cyan-400", bg: "bg-cyan-500/20", label: "Tạo yêu cầu di chuyển" };
    case "PREMIUM_MIGRATION_REQUEST_FAILED":
      return { icon: AlertTriangle, color: "text-red-400", bg: "bg-red-500/20", label: "Di chuyển thất bại" };
    default:
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {"reason" in log.details ? (
            <span className="text-slate-400 text-xs mt-0.5">
              Lý do: {String(log.details.reason)}
            </span>
          ) : null}
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2 text-xs text-slate-400">
            {"customer_name" in details ? (
              <span className="bg-slate-800/80 border border-slate-700 px-2 py-0.5 rounded-md">
                Khách: {String(details.customer_name)}
              </span>
            ) : null}
            {"service_type_id" in details ? (
              <span className="bg-slate-800/80 border border-slate-700 px-2 py-0.5 rounded-md">
                Dịch vụ: {String(details.service_type_id)}
              </span>
            ) : null}
            {"source_available_slots" in details ? (
              <span className="bg-slate-800/80 border border-slate-700 px-2 py-0.5 rounded-md">
                Kho nguồn: {String(details.source_available_slots)}
              </span>
            ) : null}
            {"target_available_slots" in details ? (
              <span className="bg-slate-800/80 border border-slate-700 px-2 py-0.5 rounded-md">
                Kho đích: {String(details.target_available_slots)}
              </span>
            ) : null}
          </div>
          {"reason" in details ? (
            <div className="text-slate-400 text-xs border-t border-slate-800 pt-1.5">
              Lý do: {String(details.reason)}
            </div>
          ) : null}
          {"notes" in details && details.notes ? (
            <div className="text-slate-500 text-xs">
              Ghi chú: {String(details.notes)}
            </div>
          ) : null}
        </div>
      </div>
  ...
  if (isError) {
    return (
      <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm">
        {error instanceof Error ? error.message : "Đã xảy ra lỗi khi tải lịch sử hoạt động."}
      </div>
    );
  }
  ...
  if (logs.length === 0) {
    return (
      <div className="text-center py-8 bg-slate-800/20 border border-slate-800 rounded-xl">
        <p className="text-slate-500 text-sm italic">Chưa có hoạt động nào được ghi nhận.</p>
      </div>
    );
  }
