...
export default function CalendarPage() {
  ...
  const handleQuickAdd = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && quickAddText.trim()) {
      ...
      try {
        ...
        appToast.success("Đã thêm công việc nhanh!");
        setQuickAddText("");
      } catch (_error) {
        appToast.error("Lỗi thêm công việc!");
      } finally {
  ...
  function handleManualRenewCancel(subscriptionId: string) {
    appToast.info(`Tính năng hủy & xóa: ${subscriptionId} (Demo)`);
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      appToast.success("Đã xóa sự kiện!");
    } catch {
      appToast.error("Lỗi xóa sự kiện");
    }
  }
  ...
  async function toggleEventDone(event: CalendarEvent) {
    try {
      ...
      if (!event.isDone) {
        appToast.success(`Đã hoàn thành: ${event.title}`);
      }
    } catch {
    ...
    <AppLayout>
      <PageContainer className="relative">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-[var(--fg-base)]">Lịch CRM</h1>
            <p className="text-[15px] text-[var(--fg-muted)] mt-1">Lịch sự kiện & theo dõi gia hạn khách hàng.</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => refetchRenewals()}
              aria-label="Tải lại danh sách gia hạn"
        ...
        <div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Sắp gia hạn (30n)", value: stats.total, color: "text-[var(--accent)]", bg: "bg-[var(--accent)]/8" },
              { label: "Hết hạn", value: stats.expired, color: "text-[var(--danger)]", bg: "bg-[var(--danger)]/8" },
              { label: "≤ 3 ngày", value: stats.urgent, color: "text-[#ff3b30]", bg: "bg-[#ff3b30]/8" },
              { label: "≤ 7 ngày", value: stats.soon, color: "text-[#ff9500]", bg: "bg-[#ff9500]/8" },
            ].map(s => (
          ...
          <div className="min-w-0">
            <div className="glass-card rounded-xl overflow-hidden border border-[var(--border-soft)] bg-white">
              ...
              <div className="flex items-center gap-4 px-4 py-2.5 border-t border-[var(--border-soft)] bg-[var(--bg-app)]/30">
                {[
                  { color: "bg-[var(--accent)]", label: "Gia hạn" },
                  { color: "bg-[#ff9500]", label: "Theo dõi" },
                  { color: "bg-[#5ac8fa]", label: "Nhắc nhở" },
                  { color: "bg-[#ff3b30]", label: "Thanh toán" },
                ].map(l => (
                ...
                <div className="flex items-center gap-1.5 ml-auto">
                  ...
                  <span className="text-[10px] text-[var(--fg-muted)] font-medium">Hạn sub</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="glass-card rounded-xl border border-[var(--danger)]/30 bg-[var(--danger)]/5 overflow-hidden flex flex-col shrink-0">
                <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--danger)]/10 bg-[var(--danger)]/10">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[13px] font-black text-[var(--danger)] tracking-tight">Quá hạn kế hoạch</h3>
                  </div>
                ...
                <div className="p-2 space-y-1 max-h-[200px] overflow-y-auto custom-scrollbar">
                  {debtEvents.filter(e => !e.isDone).length === 0 && debtEvents.length > 0 && (
                    <div className="flex flex-col items-center justify-center py-6 text-center gap-2">
                      ...
                      <p className="text-[12px] font-bold text-green-600">Đã xử lý xong nợ!</p>
                      <p className="text-[11px] font-medium text-[var(--fg-muted)]">Tuyệt vời, bạn đang làm rất tốt.</p>
                    </div>
                  )}
                  {debtEvents.map(evt => (
                    <div key={evt.id} className="flex items-start gap-2.5 p-2 rounded-lg hover:bg-white/50 transition-colors group">
                      <button onClick={(e) => { e.stopPropagation(); toggleEventDone(evt); }} className={`mt-0.5 shrink-0 transition-colors cursor-pointer ${evt.isDone ? "text-[var(--danger)]/50" : "text-[var(--danger)]/80 hover:text-[var(--danger)]"}`} aria-label="Mark done">
              ...
              <div className="relative flex flex-col h-full z-10">
                <div className="flex items-center justify-between px-4 py-3 border-b border-black/5">
                  ...
                  <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-black/5 shadow-inner">
                    ...
                    <span className="text-[9px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">Tự động lưu</span>
                  </div>
                </div>
                <textarea 
                  value={noteText}
                  onChange={(e) => setNoteText(e.target.value)}
                  placeholder="Ghi chú ý tưởng, kế hoạch..."
                ...
                return (
                  <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-soft)] bg-[var(--bg-app)]/50 backdrop-blur-sm z-10 relative">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-[13px] font-black text-[var(--fg-base)]">Hôm nay</h3>
                        {totalCount > 0 && <p className="text-[10px] font-medium text-[var(--fg-muted)]">{doneCount}/{totalCount} đã xong</p>}
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="p-2 border-b border-[var(--border-soft)] bg-[var(--surface-light)]/50 shrink-0 relative z-10">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Thêm nhanh việc và gõ Enter..."
              ...
              <div className="p-2 space-y-1 overflow-y-auto custom-scrollbar flex-1 relative min-h-[120px]">
                {todayTodoList.length === 0 ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                    ...
                    <p className="text-[12px] font-bold text-[var(--fg-muted)] text-center">Một ngày rảnh rỗi.<br/><span className="font-medium">Tận hưởng đi nhé!</span></p>
                  </div>
                ) : (
