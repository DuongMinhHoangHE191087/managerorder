...
export default function InventoryPage() {
  ...
  const filteredAccounts = useMemo(
    () =>
      sourceAccounts.filter((account) => {
        ...
        if (statusFilter === "expired" && new Date(account.expiresAt).getTime() > Date.now()) return false;
        if (statusFilter === "expiring_7d") {
          const sevenDays = 7 * 24 * 60 * 60 * 1000;
          const timeLeft = new Date(account.expiresAt).getTime() - Date.now();
    ...
    [sourceAccounts, productIdFilter, providerFilter, statusFilter, deferredSearchQuery, productMap]
  );
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    if (failed > 0) appToast.warning(`Đã xóa ${success} tài khoản, ${failed} thất bại`);
    else appToast.success(`Đã xóa ${success} tài khoản`);
  }, [selectedIds, deleteSourceAccount]);
  ...
  const handleBulkExtend = useCallback(async (days: number) => {
    ...
    const success = results.filter((result) => result.status === "fulfilled").length;
    setSelectedIds(new Set());
    if (success > 0) appToast.success(`Đã gia hạn ${days} ngày cho ${success} tài khoản`);
  }, [selectedIds, sourceAccounts, updateSourceAccount]);
  ...
  const handleBulkSync = useCallback(async () => {
    ...
    try {
      ...
      appToast.success(`Đã đồng bộ ${changed} / ${ids.length} tài khoản có thay đổi`);
    } catch {
      appToast.error("Lỗi đồng bộ hàng loạt");
    } finally {
      setIsBulkSyncing(false);
    }
  }, [selectedIds, recalculateSlots]);
  ...
  const handleExportCSV = useCallback(() => {
    if (filteredAccounts.length === 0) {
      appToast.warning("Không có dữ liệu để xuất");
      return;
    }
    const esc = (value: unknown) => `"${String(value).replace(/"/g, '""')}"`;
    const headers = ["Email", "Nhà cung cấp", "Sản phẩm", "Slot đã dùng", "Tổng slot", "Hết hạn", "Trạng thái"];
    const rows = filteredAccounts.map((account) => [
      ...
      account.usedSlots >= account.maxSlots ? "Đầy" : new Date(account.expiresAt) < new Date() ? "Hết hạn" : "Hoạt động",
    ]);
    ...
    appToast.success("Đã xuất file CSV");
  }, [filteredAccounts, providers, productMap]);

  const handleSelectedAccountRecalculate = useCallback(async () => {
    if (!selectedAccount) return;
    try {
      const result = await recalculateSlots(selectedAccount.id);
      if (result.changed) appToast.success(`Đã đồng bộ slot: ${result.previous} → ${result.recalculated}`);
      else appToast.info("Slot đã chính xác, không cần thay đổi.");
    } catch {
      appToast.error("Lỗi khi đồng bộ dữ liệu");
    }
  }, [selectedAccount, recalculateSlots]);
      ...
      <PageContainer className="relative">
        ...
        {selectedIds.size > 0 ? (
          <div className="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-3 rounded-[1.2rem] bg-[var(--fg-base)] px-5 py-3 text-white shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
            <span className="text-[13px] font-bold">{selectedIds.size} mục đã chọn</span>
            ...
            >
              <Clock className="size-3.5" /> +30 Ngày
            </button>
            <button
        ...
        <div className="app-card overflow-hidden border border-[var(--border-soft)] bg-[rgba(255,255,255,0.94)] shadow-[0_18px_44px_rgba(15,23,42,0.05)] transition-all hover:shadow-[0_22px_52px_rgba(15,23,42,0.08)]">
          <div className="flex items-center justify-between border-b border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,250,244,0.84))] p-5">
            <h3 className="text-[15px] font-bold tracking-tight text-[var(--fg-base)]">Đơn hàng chờ cấp phát</h3>
          </div>
          <div className="overflow-x-auto">
            {pendingAllocationOrders.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-10 text-center">
                <PackageSearch className="mb-3 size-10 text-[var(--fg-muted)] opacity-50" />
                <h3 className="text-[15px] font-bold text-[var(--fg-base)]">Không có đơn chờ cấp phát</h3>
              </div>
            ) : (
              <div className="flex flex-col border-t border-[var(--border-soft)]">
                <div className="hidden grid-cols-[minmax(0,2.5fr)_minmax(0,3fr)_minmax(0,2fr)_160px] items-center gap-4 border-b border-[var(--border-soft)] bg-[var(--surface-light)] px-4 py-3 lg:grid">
                  <div className="text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)]">Khách hàng / Mã Đơn</div>
                  <div className="text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)]">Sản phẩm / Doanh thu</div>
                  <div className="text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)]">Trạng thái / Thời gian</div>
                  <div className="pr-2 text-right text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)]">Thao tác</div>
                </div>
                    ...
                    return (
                      <div key={order.id} className="group flex flex-col gap-4 px-4 py-4 transition-colors hover:bg-gray-50 lg:grid lg:grid-cols-[minmax(0,2.5fr)_minmax(0,3fr)_minmax(0,2fr)_160px] lg:items-center">
                        <div className="flex min-w-0 flex-col">
                          <span className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)] lg:hidden">Khách hàng / Mã đơn</span>
                        ...
                        <div className="mt-2 flex min-w-0 flex-col border-t border-[var(--border-soft)] pt-3 lg:mt-0 lg:border-none lg:pt-0">
                          <span className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)] lg:hidden">Sản phẩm / Doanh thu</span>
                        ...
                        <div className="mt-2 flex min-w-0 flex-col items-start gap-1.5 border-t border-[var(--border-soft)] pt-3 lg:mt-0 lg:border-none lg:pt-0">
                          <span className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-[var(--fg-muted)] lg:hidden">Trạng thái / Thời gian</span>
                          <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${order.status === "paid" ? "border border-emerald-500/20 bg-emerald-500/10 text-emerald-600" : "border border-[var(--warning)]/20 bg-[var(--warning)]/10 text-[var(--warning)]"}`}>
                            {order.status === "paid" ? "Đã Thanh Toán" : order.status === "pending_payment" ? "Chưa Thanh Toán" : order.status === "provisioning" ? "Đang Xử Lý" : order.status}
                          </span>
                          <div className="mt-1 flex items-center gap-1 text-[11px] font-medium text-[var(--fg-muted)] lg:mt-0">
                            <Clock className="size-3" />
                            {order.created_at ? formatDateLabel(order.created_at) : "N/A"}
                          </div>
                        </div>
