    ...
    <div className="space-y-2">
      ...
      <div className="h-4 rounded-full bg-[var(--border-soft)] overflow-hidden flex">
        {connPct > 0 && (
          <div
            ...
            style={{ width: `${connPct}%` }}
            title={`Kết nối: ${connectedCount} slot`}
          />
        )}
        {resPct > 0 && (
          <div
            ...
            style={{ width: `${resPct}%` }}
            title={`Đặt trước: ${reservedCount} slot`}
          />
        )}
        {availPct > 0 && (
          <div
            ...
            title={`Trống: ${availableCount} slot`}
          />
        )}
      </div>
      ...
      <div className="flex items-center gap-4 text-[11px] text-[var(--fg-muted)]">
        <div className="flex items-center gap-1.5">
          ...
          <span>Kết nối ({connectedCount})</span>
        </div>
        <div className="flex items-center gap-1.5">
  ...
  const handleRecalculate = async () => {
    try {
      ...
      if (result.changed) {
        ...
        appToast.info("Slot đã chính xác, không cần thay đổi.");
      }
    } catch {
      appToast.error("Lỗi khi tính lại slot");
    }
  };
  ...
  if (isError || !data) {
    return (
      <div className="app-card border border-[var(--border-soft)] rounded-[1.15rem] p-5 text-center bg-[rgba(255,255,255,0.94)] shadow-[0_16px_38px_rgba(15,23,42,0.05)]">
        <p className="text-sm text-[var(--fg-muted)]">Không thể tải thông tin slot</p>
      </div>
    );
  }
    ...
    <div className="app-card border border-[var(--border-soft)] rounded-[1.15rem] overflow-hidden bg-[rgba(255,255,255,0.94)] shadow-[0_16px_38px_rgba(15,23,42,0.05)]">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(246,250,244,0.84))] px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-[13px] font-bold text-[var(--fg-base)]">Phân Tích Slot</h4>
        </div>
        <Button
          size="sm"
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            ...
            <div className="text-[10px] text-[var(--fg-muted)] group-hover:text-[var(--accent)] transition-colors">Kết nối</div>
          </button>
          <button
          ...
          >
            ...
            <div className="text-[10px] text-[var(--fg-muted)] group-hover:text-purple-500 transition-colors">Đặt trước</div>
          </button>
          <div className="rounded-[1rem] border border-[var(--border-soft)] bg-[var(--surface-light)]/60 p-3 text-center">
            ...
            <div className="text-[10px] text-[var(--fg-muted)]">Trống</div>
          </div>
        </div>
        ...
        {data.connectedItems.length > 0 && (
          <div className="space-y-1.5">
            ...
            {data.connectedItems.length > 3 && (
              ...
              >
                Xem tất cả {data.connectedItems.length} kết nối
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {data.reservedNicks.length > 4 && (
                ...
                >
                  +{data.reservedNicks.length - 4} nữa
                </button>
              )}
            </div>
          </div>
