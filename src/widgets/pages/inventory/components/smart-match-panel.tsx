...
interface SmartSuggestion {
  ...
  customerName: string;
  orderId: string;
  matchedField: "nick_used" | "item_notes" | "registry";
  matchedValue: string;
  confidence?: number;
}

const MATCH_LABEL: Record<SmartSuggestion["matchedField"], { label: string; icon: React.ElementType; color: string }> = {
    ...
    >
      <div className="relative w-full max-w-2xl bg-[#0d1a30] border border-slate-700/60 rounded-2xl shadow-2xl flex flex-col max-h-[88vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 custom-scrollbar">
          ...
          {/* Suggestions */}
          {visible.map(s => {
            const matchMeta = MATCH_LABEL[s.matchedField];
