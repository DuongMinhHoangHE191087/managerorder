...
export function EditSourceAccountModal({ account, onClose, providers, products, productMap, onSubmit }: EditSourceAccountModalProps) {
  ...
  // Sync state when account changes + fetch decrypted password
  useEffect(() => {
    if (account && initialized !== account.id) {
      ...
      fetch(`/api/source-accounts/${account.id}/decrypt`)
        .then(r => r.ok ? r.json() : null)
        .then(json => {
          if (json?.data) {
            if (json.data.password) setPassword(json.data.password);
            if (json.data.credentials?.length) setCredentials(json.data.credentials);
          }
        })
