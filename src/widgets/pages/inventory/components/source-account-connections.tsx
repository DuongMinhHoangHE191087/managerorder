...
export function SourceAccountConnections({ 
  ...
  const handleDisconnectClick = (item: ConnectionItem) => {
    if (confirm(`Bạn có chắc chắn muốn ngắt kết nối với đơn hàng ${item.orders.id.split('-')[0]}?`)) {
  ...
  const handleDisconnect = async (orderItemId: string, quantity: number) => {
    try {
      ...
      appToast.success("Đã ngắt kết nối thành công");
    } catch (err: unknown) {
      appToast.error(err instanceof Error ? err.message : "Lỗi khi ngắt kết nối");
    }
  };

  const handleReconnect = async (orderItemId: string, quantity: number) => {
    if (usedSlots + quantity > maxSlots) {
      appToast.error("Không đủ slot trống để kết nối");
      return;
    }
    try {
      await reconnect({ sourceAccountId, orderItemId, quantity });
      appToast.success("Đã kết nối lại thành công");
    } catch (err: unknown) {
      appToast.error(err instanceof Error ? err.message : "Lỗi khi kết nối");
    }
  };
  ...
  const handleSyncSlots = async () => {
    try {
      ...
      if (result.changed) {
        appToast.success(`Đã đồng bộ slot: ${result.previous} → ${result.recalculated}`);
      } else {
        appToast.info("Slot đã chính xác, không cần đồng bộ");
      }
    } catch (err: unknown) {
      appToast.error(err instanceof Error ? err.message : "Lỗi khi đồng bộ slot");
    }
  };
  ...
  if (error || !data) {
    return <div className="p-4 bg-red-50 text-red-600 rounded-lg text-sm text-center">Lỗi tải dữ liệu kết nối</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[1rem] border border-[var(--border-soft)] bg-[var(--surface-light)]/70 p-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[11px] font-bold text-[var(--fg-muted)] uppercase tracking-wider">Phân bổ Slot</span>
          <div className="flex items-center gap-2">
            <button
              ...
              title="Đồng bộ lại slot từ dữ liệu thực tế"
            >
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px]">
          <span className="flex items-center gap-1">
            ...
            <span className="text-[var(--fg-muted)]">Kết nối: <strong className="text-[var(--fg-base)]">{connectedSlots}</strong></span>
          </span>
          <span className="flex items-center gap-1">
            <span className="inline-block size-2 rounded-full bg-purple-400" />
            <span className="text-[var(--fg-muted)]">Đặt trước: <strong className="text-[var(--fg-base)]">{reservedCount}</strong></span>
          </span>
          <span className="flex items-center gap-1">
            <span className="inline-block size-2 rounded-full bg-gray-300" />
            <span className="text-[var(--fg-muted)]">Trống: <strong className="text-[var(--fg-base)]">{freeSlots}</strong></span>
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {connected.length === 0 ? (
          <p className="text-[12px] text-[var(--fg-muted)] text-center py-4 bg-gray-50 rounded-xl border border-dashed border-[var(--border-soft)]">Chưa có khách hàng nào kết nối</p>
        ) : (
              ...
              <FadeIn key={item.id} className="flex items-center justify-between gap-3 rounded-[1rem] border border-[var(--border-soft)] bg-[rgba(255,255,255,0.96)] p-3 shadow-[0_10px_24px_rgba(15,23,42,0.04)] transition-colors hover:border-[var(--accent)]/30">
                <div className="flex flex-col gap-1 min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-[var(--fg-muted)] flex items-center gap-2">
                    <span>Mã ĐH: {item.orders.id.split('-')[0]}</span>
                    <span>•</span>
      ...
      <div className="space-y-3">
        <h4 className="text-[12px] font-bold text-[var(--fg-muted)] uppercase tracking-wider flex items-center gap-2 mt-6">
          ...
          Chờ kết nối ({unconnected.length})
        </h4>
        {unconnected.length === 0 ? (
          <p className="text-[12px] text-[var(--fg-muted)] text-center py-4 bg-gray-50 rounded-xl border border-dashed border-[var(--border-soft)]">Không có đơn hàng nào chờ</p>
        ) : (
          <div className="space-y-2">
      ...
      {reservedNicks.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-[12px] font-bold text-purple-400/80 uppercase tracking-wider flex items-center gap-2 mt-6">
            ...
            Nick đặt trước ({reservedNicks.length})
          </h4>
          ...
          <p className="text-[10px] text-slate-600 italic">
            Smart Match sẽ ưu tiên gợi ý kết nối khi khách dùng nick đã đặt trước.
          </p>
        </div>
      )}
    </div>
