...
## ⚡ RTK Token Saver
1. Khi chạy lệnh shell, **ưu tiên `rtk`** cho `git`, `npm`, `npx`, `cargo`, `test`, `build`, `lint`.
2. Khi tìm file hoặc text, **ưu tiên `rg`** và `Get-ChildItem -Force`.
3. Khi trả lời, chỉ giữ **kết quả cần thiết**; không dán log dài nếu không cần cho debug.
4. Nếu lệnh sinh output lớn, chuyển sang `rtk` hoặc `rtk summary` trước khi đọc.
